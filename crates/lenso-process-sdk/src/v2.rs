...
pub trait ProcessPluginV2: Send + Sync + 'static {
    ...
    fn construct(&self, params: &ConstructParams) -> Result<Self::Instance, String>;
...
/// Per-invocation authority for cancellation observation and exact outbound routes.
#[derive(Clone)]
pub struct ProcessInvocationContext {
...
impl std::fmt::Debug for ProcessInvocationContext {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("ProcessInvocationContext")
...
fn serve_v2_io<P: ProcessPluginV2>(
    ...
    let initialize = match read_v2_frame(&mut reader, max_frame_bytes)? {
    ...
    while let Some(frame) = read_v2_frame(&mut reader, max_frame_bytes)? {
        match frame {
            ...
            HostFrameV2::Construct(params) => {
                ...
                let outcome = match plugin.construct(&params) {
                    Ok(value) => {
            ...
            HostFrameV2::Stop(params) => {
                ...
                let (hook, diagnostics) = match plugin.stop(object, &params) {
