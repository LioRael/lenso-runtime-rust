...
mod tests {
    ...
    fn wire(fields: Value) -> String {
        let mut input = json!({"method":"POST","uri":"/bytes?","headers":[["x-test","one"],["x-test","two"]]});
        input.as_object_mut().unwrap().extend(fields.as_object().unwrap().clone());
        input.to_string()
    }
    ...
    fn canonical_and_legacy_bodies_preserve_bytes_and_head() {
        for size in [0, 1, 2, 3, 256, BODY_LIMIT - 2, BODY_LIMIT - 1, BODY_LIMIT] {
            ...
            for fields in [json!({"body":bytes}), json!({"body_encoding":"base64-v1","body_base64":STANDARD.encode(&bytes)})] {
    ...
    fn rejects_noncanonical_base64() {
        for encoded in ["AB==", "AAB=", "AA", "AAA", "AA-_", "AA==\n", "AA ==", "====", "A===", "AA=A", "AA==AAAA", "éAAA"] {
            assert!(decode_request(&wire(json!({"body_encoding":"base64-v1","body_base64":encoded}))).is_err(), "{encoded:?}");
        }
    }
    ...
    fn rejects_missing_ambiguous_null_or_unknown_fields() {
        for fields in [
            json!({}), json!({"body_base64":""}), json!({"body_encoding":"base64-v1"}),
            ...
            json!({"body":[],"body_encoding":null}), json!({"body":[],"body_base64":null}),
            json!({"body_encoding":"base64-v1","body_base64":null}),
            json!({"body_encoding":null,"body_base64":""}),
            json!({"body":null}), json!({"body":[],"extra":true}),
            json!({"body_encoding":"base64-v1","body_base64":[]}),
            json!({"body":[256]}), json!({"body":[-1]}), json!({"body":[1.5]}),
        ] {
            assert!(decode_request(&wire(fields.clone())).is_err(), "{fields}");
        }
        for fields in [r#""body":[],"body":[]"#, r#""body_encoding":"base64-v1","body_base64":"","body_base64":"""#, r#""body_encoding":"base64-v1","body_encoding":"base64-v1","body_base64":"""#] {
            assert!(decode_request(&format!(r#"{{"method":"POST","uri":"/bytes","headers":[],{fields}}}"#)).is_err());
        }
    }
    ...
    fn bounds_decoded_bytes_even_when_encoded_length_fits() {
        ...
        for size in [BODY_LIMIT + 1, BODY_LIMIT + 2] {
            ...
            let error = decode_request(&wire(json!({"body_encoding":"base64-v1","body_base64":encoded}))).unwrap_err();
    ...
    fn bounds_encoded_allocation_and_legacy_body() {
        ...
        assert_eq!(decode_request(&wire(json!({"body_encoding":"base64-v1","body_base64":encoded}))).unwrap_err(), "encoded request body exceeds bound");
        assert_eq!(decode_request(&wire(json!({"body":vec![0; BODY_LIMIT + 1]}))).unwrap_err(), "request body exceeds bound");
        assert_eq!(decode_request(&" ".repeat(BODY_LIMIT * 4 + HEAD_LIMIT * 6 + 1)).unwrap_err(), "serialized request exceeds bound");
    }
}
