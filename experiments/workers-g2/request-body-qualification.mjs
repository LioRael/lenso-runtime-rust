// Shared, real Rust/Wasm assertions executed by Node and workerd.
...
export function requestBodyQualification(test, bindings, module, clearTimers) {
  ...
  for (const encoding of ["numeric-array", "base64-v1"]) {
    ...
    test(`${encoding}: a real held session rejects both transports before reading or encoding`, { timeout: 5000 }, async () => {
      const held = await host.streaming(new Request("https://proof.invalid/stream?hold"));
      assert.equal(held.status, 200);
      const originalBtoa = globalThis.btoa;
      let encodes = 0;
      globalThis.btoa = (...args) => { encodes++; return originalBtoa(...args); };
      try {
  ...
  test("both encodings produce identical HTTP status, headers and bytes", async () => {
    async function snapshot(response) {
      return {
        status: response.status,
        headers: [...response.headers].filter(([name]) => name !== "x-request-id"),
        body: await bytesOf(response),
      };
    }
    for (const kind of ["buffered", "streaming"]) {
      const requests = kind === "buffered"
        ? [() => post(new TextEncoder().encode("你好 🌍 café\u0000")), () => post(new Uint8Array([0, 255, 128]))]
        : [stream, () => new Request("https://proof.invalid/stream", { method: "POST", body: new Uint8Array([0, 255]) })];
      for (const request of requests) {
        const numeric = await snapshot(await handlers("numeric-array")[kind](request()));
        const base64 = await snapshot(await handlers("base64-v1")[kind](request()));
        assert.deepEqual(base64, numeric);
      }
    }
  });
  ...
  test("both actual Rust entry points reject malformed envelopes with bounded HTTP errors and recover", { timeout: 30000 }, async () => {
    ...
    const malformed = [
      ...
      ...[{ "base64-v1": null }, ["base64-v1"], 1, true]
        .map((body_encoding) => ({ body_encoding, body_base64: "AA==" })),
    ...
    malformed.push(
      ...
      '{"method":"POST","uri":"/bytes","headers":[],"body_encoding":"base64-v1","body_encoding":"base64-v1","body_base64":""}',
