import { disconnectProof } from './disconnect-proof.mjs';
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname.startsWith('/disconnect')) return disconnectProof(request, env, ctx, boot);
    ...
    if (input.length > 1024 || !['normal', 'missing-factory', 'startup-failure', 'trap', 'driver', 'async-trap', 'task-trap', 'async-panic', 'async-pending', 'recovery', 'conformance', 'lifecycle', 'named-dependencies', 'diagnostics', 'interactions', 'schedules', 'bindings', 'io-exchange', 'io-delayed', 'io-slow', 'io-cancel', 'io-recovery'].includes(mode)) return new Response('Invalid probe', { status: 400 });
