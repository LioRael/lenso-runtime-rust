...
const denied = await fetch(new URL('/memory', base));
assert.equal(denied.status, 404);
const before = await healthy();
const failure = await fetch(new URL('/memory', base), options());
...
console.log(JSON.stringify({ passed: true, http_status: failure.status, platform_error: 1102, fault: 'memory-pressure', cpu_limit_ms: 1000, before_boot: before.boot, after_boot: after.boot, following_request: 'healthy', forced_shutdown: 'unconfirmed', note: 'Boot IDs are observations across HTTP requests, not proof that the same isolate was selected or evicted.' }));
