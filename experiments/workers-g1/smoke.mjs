...
console.log(JSON.stringify({ base, passed: true, concurrent_requests: inputs.length, checks: ['static registration', 'Ready', 'bound Capability invocation', 'event state isolation', 'timer', 'task cancellation', 'missing factory', 'prepare failure', 'clean shutdown', 'recreation', 'bounded tasks', 'shutdown wakes parked lane', 'cancelled invocation', 'expired invocation', 'synchronous Wasm trap rejection'] }));
